...
def detectron_tst(p_train: Dataset,
    ...
    trainer = pl.Trainer(accelerator= "auto", max_epochs=max_epochs_per_model, **trainer_kwargs)
...
def infer_labels(model: torch.nn.Module, dataset: Dataset | Sequence[Dataset], batch_size: Optional[int] = None,
    ...
    tr = pl.Trainer(accelerator='auto', max_epochs=1, enable_model_summary=False, logger=False)
