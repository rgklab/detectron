...
for test in [test_p, test_q]:
    for d in test:
        for dd in d:
            #first solution using test_probabilities
            dd['test_probabilities'] = dd['test_probabilities'].numpy()
            #second solution using logits
...
for N in Ns:
    ...
    for seed in tp.query(f'N=={N}').seed.unique():
        test_probabilities = tp.query(f'seed=={seed} and N=={N}').iloc[:n + 1].test_probabilities
        mean_probs = np.mean(test_probabilities)  
        #second solution using the logits
        #mean_probs = tq.query(f'seed=={seed} and N=={N}').iloc[:n + 1].logits.map(lambda x: sigmoid(x)).mean()
        entropy = -(mean_probs * np.log2(mean_probs + 1e-15) + (1 - mean_probs) * np.log2(1 - mean_probs + 1e-15))
    ...
    for seed in tq.query(f'N=={N}').seed.unique():
        test_probabilities = tq.query(f'seed=={seed} and N=={N}').iloc[:n + 1].test_probabilities
        mean_probs = np.mean(test_probabilities)  
        #second solution using the logits
        #mean_probs = tq.query(f'seed=={seed} and N=={N}').iloc[:n + 1].logits.map(lambda x: sigmoid(x)).mean()
        entropy = -(mean_probs * np.log2(mean_probs + 1e-15) + (1 - mean_probs) * np.log2(1 - mean_probs + 1e-15))
